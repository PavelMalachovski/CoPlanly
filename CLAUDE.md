...
- **GitHub CI runs on every pull request, and on every push to `main`** — a push to a
  ...
  August 2026 — this line used to say there was none). Eight jobs that test (this line used to
  say seven, before `instrumented` was added), plus `report`, which only reads them (below):
  `changes` (a cheap gate,
