...
- **GitHub CI runs on every pull request, and on every push to `main`** — a push to a
  ...
  seven, before `instrumented` was added, and eight before `e2e`): `changes` (a cheap gate,
  ...
  *app's* policy. **Two JDKs**: the emulators take 21 through `FIREBASE_JAVA_HOME`, Gradle stays
  ...
  push is not sent), anything drawn on screen, and `ChatViewModel`, which the known issue on chat
  and the first co-parent says still follows the server's first co-parent.
